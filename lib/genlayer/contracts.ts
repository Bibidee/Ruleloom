...
export async function applicationCount(){assertConfigured();return publicClient.readContract({address:addresses.book,functionName:'get_application_count'}) as Promise<bigint>}
export async function evaluationCount(){assertConfigured();return publicClient.readContract({address:addresses.book,functionName:'get_evaluation_count'}) as Promise<bigint>}
export async function passCount(){assertConfigured();return publicClient.readContract({address:addresses.pass,functionName:'get_pass_count'}) as Promise<bigint>}
export async function verifyBindings(){assertConfigured();const [boundPass,boundBook]=await Promise.all([publicClient.readContract({address:addresses.book,functionName:'get_passbook'}),publicClient.readContract({address:addresses.pass,functionName:'get_book_address'})]);return String(boundPass).toLowerCase()===addresses.pass.toLowerCase()&&String(boundBook).toLowerCase()===addresses.book.toLowerCase()}
