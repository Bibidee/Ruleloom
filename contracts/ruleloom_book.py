...
def _canonical_url(url):
    ...
    try:
        ...
        if address.version!=4 or not address.is_global or address.is_private or address.is_loopback or address.is_link_local or address.is_reserved or address.is_unspecified or address.is_multicast or getattr(address,"is_site_local",False) or str(address).startswith("100.") and 64<=int(str(address).split(".")[1])<=127: raise gl.vm.UserError("private URL rejected")
    except ValueError:
        labels=host.split(".")
